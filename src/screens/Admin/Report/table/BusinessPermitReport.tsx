...
const BusinessPermitReport = forwardRef<HTMLDivElement, TableReportProps>(({
  ...
  const [showLoader, setShowLoader] = useState(() => {
    return !isSameFilter(
      { selectedRegions, selectedProvinces, selectedCities, selectedIslands: islandsToUse, dateRange },
      persistedAppliedFilter
    );
  });

  useEffect(() => {
    setShowLoader(!isSameFilter(
      { selectedRegions, selectedProvinces, selectedCities, selectedIslands: islandsToUse, dateRange },
      persistedAppliedFilter
    ));
  }, [persistedTableData, persistedAppliedFilter, selectedRegions, selectedProvinces, selectedCities, islandsToUse, dateRange]);

  const effectiveApiData = useMemo(() => {
    return isSameFilter(
        { selectedRegions, selectedProvinces, selectedCities, selectedIslands: islandsToUse, dateRange },
        persistedAppliedFilter
    ) ? persistedTableData : apiData;
  }, [apiData, persistedTableData, persistedAppliedFilter, selectedRegions, selectedProvinces, selectedCities, islandsToUse, dateRange]);
