...
const BuildingPermitReport = forwardRef<HTMLDivElement, BuildingPermitProps>(({
  ...
  const grandTotals = useMemo(() => { const totals = { paid: 0, pending: 0 }; normalizedResults.forEach((lgu: any) => { if (!lgu.hasError && lgu.monthlyResults && Array.isArray(lgu.monthlyResults)) { lgu.monthlyResults.forEach((item: any) => { totals.paid += Number(item.buildingPaid || 0); totals.pending += Number(item.buildingPending || 0); }); } }); return totals; }, [normalizedResults]);
    ...
    sortedRegionKeys.forEach(region => {
      ...
      if (isRegionOpen) {
        ...
        lguList.forEach((lgu: any) => {
          ...
          const dataToRender = isDayMode ? (lgu.monthlyResults?.length > 0 ? lgu.monthlyResults : [{ buildingPaid: 0, buildingPending: 0 }]) : [ (lgu.monthlyResults || []).reduce((acc: any, current: any) => { acc.buildingPaid += Number(current.buildingPaid || 0); acc.buildingPending += Number(current.buildingPending || 0); return acc; }, { buildingPaid: 0, buildingPending: 0 })];
          dataToRender.forEach((item: any, itemIdx: number) => {
            ...
            rows.push(
              <TableRow key={`${region}-${lgu.lgu}-${item.month || itemIdx}`} className="hover:bg-blue-50/70 transition-colors duration-200 text-sm">
                ...
                <TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.buildingPaid)}</TableCell>
                <TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.buildingPaid)}</TableCell>
                <TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.buildingPending)}</TableCell>
                <TableCell className="p-3 text-right font-bold tabular-nums text-slate-900 bg-slate-100">{formatNumber(Number(item.buildingPaid || 0) + Number(item.buildingPending || 0))}</TableCell>
              </TableRow>
            );
          ...
          <Table className="w-full border-collapse" containerClassName="max-h-[70vh]">
            ...
            <TableBody className="[&>tr:nth-child(even)]:bg-white [&>tr:nth-child(odd)]:bg-slate-50/50">
              {normalizedResults.length > 0 ? (
                <>
                  {renderTableRows()}
                  {Object.keys(regionMappingGrouped).length > 1 && <TableRow><TableCell colSpan={6} className="text-center p-2 cursor-pointer bg-slate-100 hover:bg-slate-200 font-bold text-slate-700 text-xs" onClick={toggleAllRegions}>{openRegions.size === Object.keys(regionMappingGrouped).length ? 'Collapse All Regions' : 'Expand All Regions'}</TableCell></TableRow>}
                  {(loading || isProgressive) && <TableRow><TableCell colSpan={6} className="p-0"><LoaderTable message={isProgressive ? "Please wait for other regions..." : "Updating data..."} /></TableCell></TableRow>}
                </>
              ) : loading ? (
                <TableRow><TableCell colSpan={6} className="text-center py-12"><LoaderTable /></TableCell></TableRow>
              ) : (
                <TableRow>
