      ...
      if (isRegionOpen) {
        ...
        lguList.forEach((lgu: any) => {
          ...
          dataToRender.forEach((item: any, itemIdx: number) => {
            ...
            rows.push(<TableRow key={`${region}-${lgu.lgu}-${isDayMode ? item.month : 'sum'}-${itemIdx}`} className="hover:bg-blue-50/70 transition-colors duration-200 text-sm">{isFirstRowOfRegion && <TableCell className="p-3 text-center font-bold text-slate-700 align-middle bg-slate-50 border-r text-sm" rowSpan={totalRowsForRegion}>{getRegionCode(region) || region}</TableCell>}<TableCell className="p-3 text-left font-semibold text-slate-800"><div>{lgu.lgu}<span className="text-xs font-medium text-slate-500 ml-1.5">{lgu.province ? `(${lgu.province})` : ""}</span></div><div className="text-[11px] font-semibold text-blue-700 mt-0.5">{periodLabel}</div></TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.newPaid)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.newPaid)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.newPaidViaEgov)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.newPending)}</TableCell><TableCell className="p-3 text-right font-bold text-slate-900 bg-slate-100 tabular-nums">{formatNumber((item.newPaid || 0) + (item.newPaidViaEgov || 0) + (item.newPending || 0))}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.renewPaid)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.renewPaid)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.renewPaidViaEgov)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.renewPending)}</TableCell><TableCell className="p-3 text-right font-bold text-slate-900 bg-slate-100 tabular-nums">{formatNumber((item.renewPaid || 0) + (item.renewPaidViaEgov || 0) + (item.renewPending || 0))}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.malePaid)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.malePaid)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.malePending)}</TableCell><TableCell className="p-3 text-right font-bold text-slate-900 bg-slate-100 tabular-nums">{formatNumber((item.malePaid || 0) + (item.malePending || 0))}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.femalePaid)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.femalePaid)}</TableCell><TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.femalePending)}</TableCell><TableCell className="p-3 text-right font-bold text-slate-900 bg-slate-100 tabular-nums">{formatNumber((item.femalePaid || 0) + (item.femalePending || 0))}</TableCell></TableRow>);
          ...
          <Table className="w-full border-collapse" containerClassName="max-h-[70vh]">
            ...
            <TableBody className="[&>tr:nth-child(even)]:bg-white [&>tr:nth-child(odd)]:bg-slate-50/50">
              {normalizedResults.length > 0 ? (
                ...
                <TableRow><TableCell colSpan={20} className="text-center py-20 bg-white"><div className='flex flex-col items-center justify-center'><div className="rounded-full bg-slate-100 p-4"><Search className="h-10 w-10 text-slate-400" /></div><p className='font-bold text-lg text-slate-600 mt-5'>{hasSearched ? 'No Results Found' : 'Generate a Report'}</p><p className="text-sm text-slate-500 mt-1 max-w-md mx-auto">{hasSearched ? 'There is no data matching your selected filters. Please try adjusting your criteria.' : 'Use the filters above to generate your business permit report.'}</p></div></TableCell></TableRow>
              )}
            </TableBody>
