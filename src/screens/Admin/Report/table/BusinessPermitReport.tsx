...
const BusinessPermitReport = forwardRef<HTMLDivElement, BusinessPermitProps>(({ 
  ...
  const grandTotals = useMemo(() => {
    const totals = { citizensServed: 0, newIssued: 0, newPaid: 0, newGeoPay: 0, newPending: 0, renewalIssued: 0, renewalPaid: 0, renewalGeoPay: 0, renewalPending: 0, maleIssued: 0, malePaid: 0, malePending: 0, femaleIssued: 0, femalePaid: 0, femalePending: 0 };
    normalizedResults.forEach((lgu: any) => {
      if (!lgu.hasError && Array.isArray(lgu.monthlyResults)) {
        ...
        lgu.monthlyResults.forEach((item: any) => {
          totals.newIssued += getLicenseIssued(item, NEW_LICENSE_KEYS, Number(item.newPaid || 0));
          ...
          totals.renewalIssued += getLicenseIssued(item, RENEW_LICENSE_KEYS, Number(item.renewPaid || 0));
          ...
          <Table className="w-full border-collapse" containerClassName="max-h-[70vh]">
            ...
            <TableBody className="[&>tr:nth-child(even)]:bg-white [&>tr:nth-child(odd)]:bg-slate-50/50">
              {normalizedResults.length > 0 ? (
                ...
                <TableRow><TableCell colSpan={23} className="text-center py-20 bg-white"><div className='flex flex-col items-center justify-center'><div className="rounded-full bg-slate-100 p-4"><Search className="h-10 w-10 text-slate-400" /></div><p className='font-bold text-lg text-slate-600 mt-5'>{hasSearched ? 'No Results Found' : 'Generate a Report'}</p><p className="text-sm text-slate-500 mt-1 max-w-md mx-auto">{hasSearched ? 'There is no data matching your selected filters. Please try adjusting your criteria.' : 'Use the filters above to generate your business permit report.'}</p></div></TableCell></TableRow>
              )}
            </TableBody>
