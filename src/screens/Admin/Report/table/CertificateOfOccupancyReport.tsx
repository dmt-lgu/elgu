...
const CertificateOfOccupancyReport = forwardRef<HTMLDivElement, CertificateOfOccupancyProps>(({
  ...
  const grandTotals = useMemo(() => { const totals = { paid: 0, pending: 0, total: 0 }; normalizedResults.forEach((lgu: any) => { if (!lgu.hasError && Array.isArray(lgu.monthlyResults)) { lgu.monthlyResults.forEach((item: any) => { const paid = Number(item.coPaid || 0); const pending = Number(item.coPending || 0); totals.paid += paid; totals.pending += pending; totals.total += paid + pending; }); } }); return totals; }, [normalizedResults]);
          ...
          dataToRender.forEach((item: any, itemIdx: number) => {
            ...
            rows.push(
              <TableRow key={`${region}-${lgu.lgu}-${item.month || itemIdx}`} className="hover:bg-blue-50/70 transition-colors duration-200 text-sm">
                ...
                <TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.coPaid)}</TableCell>
                <TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.coPaid)}</TableCell>
                <TableCell className="p-3 text-right tabular-nums text-slate-800">{formatNumber(item.coPending)}</TableCell>
                <TableCell className="p-3 text-right font-bold tabular-nums text-slate-900 bg-slate-100">{formatNumber((Number(item.coPaid || 0) + Number(item.coPending || 0)))}</TableCell>
              </TableRow>
            );
