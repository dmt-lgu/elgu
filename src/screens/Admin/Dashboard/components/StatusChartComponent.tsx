    ...
    <table className="min-w-full border rounded bg-card text-xs">
      <thead>
        <tr>
          ...
          {raw[0]?.data.map((region: any) => (
            <th key={region.name} className="px-2 py-1 bg-primary/40  border uppercase text-left">
              {region.name}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {raw.map((period: any, idx: number) => (
          <tr key={period.date} className={idx % 2 === 0 ? 'bg-[#2222ff08] border-b ' : 'bg-[#ffffff] border-b ' }>
            ...
            {period.data.map((item: any) => {
              const prev = raw[idx - 1]?.data?.find((d: any) => d.name === item.name);
              ...
              return (
                <td key={item.name} className="px-2 py-1 align-top">
                  <div>
